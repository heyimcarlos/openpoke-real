...
- **Blocking**: use GitHub's native issue dependencies. Add an edge with `gh api --method POST repos/<owner>/<repo>/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`. The blocker ID is its numeric database ID, obtained with `gh api repos/<owner>/<repo>/issues/<n> --jq .id`.
- **Frontier query**: list the map's open children, then exclude claimed tickets and tickets with open blockers. The first remaining ticket in map order wins.
